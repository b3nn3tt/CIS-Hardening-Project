...
#                                                                      #
# File Name:   nix_ensure_squashfs_filesystem_disabled.sh              #
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure mounting of squashfs filesystems is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      l_output="" 
      ...
      if [ "$l_loadable_test" = "passed" ] && [ "$l_loaded_test" = "passed" ] && [ "$l_denied_test" = "passed" ]; then
			...
			echo -e "- End check - Ensure mounting of squashfs filesystems is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure mounting of squashfs filesystems is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"

      # Update modprobe.d file
      if ! grep -E "^install\s+$XCCDF_VALUE_REGEX\s+\/bin\/(true|false)\b" /etc/modprobe.d/*; then
         echo -e "- Adding 'install /bin/true' to  /etc/modprobe.d/$XCCDF_VALUE_REGEX.conf"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if ! grep -E "^blacklist\s+$XCCDF_VALUE_REGEX" /etc/modprobe.d/*; then
         echo -e "- Adding 'blacklist' to  /etc/modprobe.d/$XCCDF_VALUE_REGEX.conf"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ -n "$(lsmod | grep $XCCDF_VALUE_REGEX)" ]; then
         echo -e "- Unloading $XCCDF_VALUE_REGEX module"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         modprobe -r "$XCCDF_VALUE_REGEX" && test=remediated
      fi

      echo -e "- End remediation - Ensure mounting of squashfs filesystems is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
