...
#                                                                        #
# File Name:   RHEL9_2.2.10_ensure_samba_not_installed.sh                #
# Description: Ensures Samba is not installed                            #
#                                                                        #
########################################################################## 
...
ensure_samba_not_installed()
{
	...
	ensure_samba_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ samba | grep "not installed"; then
						echo -e "- PASSED:\n- samba package not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- samba package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ samba; then	
						...
						echo -e "- PASSED:\n- samba package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure Samba is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
