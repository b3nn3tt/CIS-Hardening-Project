...
#                                                                                  #
# File Name:   RHEL9_3.4.2.4_fed_ensure_nftables_loopback_traffic_is_configured.sh #
# Description: Ensures nftables loopback traffic is configured                     #
#                                                                                  #
####################################################################################
...
fed_ensure_nftables_loopback_traffic_is_configured()
{
   ...
   ipv6_chk()
   {
	...
	{
		echo "- Start check - Ensure nftables loopback traffic is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		output=""
      ...
      if [ -n "$l_v4_lo_rule" ]; then
         echo "- Found INPUT rule for loopback interface" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         if [ -n "$l_v4_127_rule" ]; then
            echo "- Found INPUT rule for loopback network" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_v4_ruleset=passed
         else
            echo "- NO INPUT rule for loopback network" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         fi
      else
         echo "- NO INPUT rule for loopback interface" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      fi
      ...
      if [ "$G_IPV6" = true ]; then
         ...
         if [ -n "$l_v6_1_rule" ]; then
            echo "- Found IPv6 INPUT rule for loopback address" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_v6_ruleset=passed
         else
            echo "- NO IPv6 INPUT rule for loopback address" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         fi
      else
         l_v6_ruleset=passed
      fi
      ...
      if [ "$l_v4_ruleset" = passed ] && [ "$l_v6_ruleset" = passed ] ; then
         echo "- The nftables rules for loopback are configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         echo "- End check - Ensure firewalld is either not installed or masked with iptables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         return "${XCCDF_RESULT_PASS:-101}"
      else
         # print the reason why we are failing
         echo "- The nftables rules for loopback are NOT configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         echo "- End check - Ensure firewalld is either not installed or masked with iptables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         return "${XCCDF_RESULT_FAIL:-102}"
      fi
	}
	...
	{
		echo -e "- Start remediation - Ensure nftables loopback traffic is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		
      echo -e "- Add rules to allow the loopback interface to accept traffic according to your local site policies." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      echo -e "- These example rules can be used if desired:\n nft add rule inet filter input iif lo accept\n nft create rule inet filter input ip saddr 127.0.0.0/8 counter drop\n nft add rule inet filter input ip6 saddr ::1 counter drop" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      test=manual
      
		echo -e "- End remediation - Ensure nftables loopback traffic is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
