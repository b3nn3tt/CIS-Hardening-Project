...
#                                                                            #
# File Name:   RHEL9_3.4.2.2_fed_ensure_nftables_table_exists.sh             #
# Description: Ensures an nftables table exists                              #
#                                                                            #
##############################################################################
...
fed_ensure_nftables_table_exists()
{
	...
	fed_ensure_nftables_table_exists_chk()
	{
      ...
      l_nftables=$(nft list tables)
		...
		if [ -n "$l_nftables" ] ; then
			echo "- An nftables table exists:\n$l_nftables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure an nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
         echo -e "- No nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure an nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	...
	fed_ensure_nftables_table_exists_fix()
	{
      ...
      echo -e "- Run \"nft create table inet <table name>\" to create a table according to your local site policies." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      test=manual
