...
#                                                                      #
# File Name:   RHEL9_1.1.2.3_ensure_noexec_set_tmp_partition.sh        #
# Description: Ensures noexec option set on /tmp partition             #
#                                                                      #
########################################################################
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure noexec option set on /tmp partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure noexec option set on /tmp partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure noexec option set on /tmp partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure noexec option set on /tmp partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
