...
#                                                                      #
# File Name:   RHEL9_1.8.2_ensure_gdm_login_banner_configured.sh       #
# Description: Ensures GDM login banner is configured                  #
#                                                                      #
########################################################################
...
ensure_gdm_login_banner_configured()
{
	...
	{
		echo "- Start check - Ensure GDM login banner is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        l_pkgoutput=""
        ...
        if [ -n "$l_pkgoutput" ]; then
            if [ -z "$l_output2" ]; then
                ...
                echo -e "- End check - Ensure GDM login banner is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            ...
            echo -e "- GNOME Desktop Manager is NOT installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            echo -e "- End check - Ensure GDM login banner is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_test="NA"
        fi
    }
    ...
    ensure_gdm_login_banner_configured_fix()
	{
        ...
        echo -e "- Start remediation - Ensure GDM login banner is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"      
        ...
        echo -e "- End remediation - Ensure GDM login banner is configured" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
