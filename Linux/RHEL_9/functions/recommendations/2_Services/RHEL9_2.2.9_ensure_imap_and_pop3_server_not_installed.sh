...
#                                                                        #
# File Name:   RHEL9_2.2.9_ensure_imap_and_pop3_server_not_installed.sh  #
# Description: Ensures IMAP and POP3 server are not installed            #
#                                                                        #
##########################################################################
...
ensure_imap_and_pop3_server_not_installed()
{
	...
	ensure_imap_and_pop3_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ dovecot | grep "not installed" && $G_PQ cyrus-imapd | grep "not installed"; then
						echo -e "- PASSED:\n- dovecot and cyrus-imapd packages not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- dovecot and cyrus-imapd packages installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ dovecot-imapd && $G_PQ dovecot-pop3d; then	
						...
						echo -e "- FAILED:\n- dovecot-imapd and dovecot-pop3d package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- dovecot-imapd and dovecot-pop3d package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure IMAP and POP3 server are not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
