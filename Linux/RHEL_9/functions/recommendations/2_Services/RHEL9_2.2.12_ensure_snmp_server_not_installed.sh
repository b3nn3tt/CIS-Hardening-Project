...
#                                                                        #
# File Name:   RHEL9_2.2.12_ensure_snmp_server_not_installed.sh          #
# Description: Ensures SNMP Server is not installed                      #
#                                                                        #
##########################################################################
...
ensure_snmp_server_not_installed()
{
	...
	ensure_snmp_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ net-snmp | grep "not installed"; then
						echo -e "- PASSED:\n- net-snmp package not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- net-snmp package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ snmp; then	
						...
						echo -e "- FAILED:\n- snmp package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- snmp package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure SNMP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
