...
#                                                                      #
# File Name:   RHEL9_1.8.4_ensure_gdm_screen_locks_when_user_idle.sh   #
# Description: Ensures GDM screen locks when the user is idle          #
#                                                                      #
########################################################################
...
ensure_gdm_screen_locks_when_user_idle()
{
	...
	{
		echo "- Start check - Ensure GDM screen locks when the user is idle" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        l_pkgoutput=""
        ...
        if [ -n "$l_pkgoutput" ]; then
            ...
            echo -e "- End check - Ensure GDM screen locks when the user is idle" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_test="NA"
        fi
    ...
    ensure_gdm_screen_locks_when_user_idle_fix()
	{
        ...
        echo -e "- Start remediation - Ensure GDM screen locks when the user is idle" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"      
      
        l_gdmprofile="gdm"

        if [ ! -f "/etc/dconf/profile/$l_gdmprofile" ]; then
            echo -e "Creating profile \"$l_gdmprofile\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            echo -e "user-db:user\nsystem-db:$l_gdmprofile\nfile-db:/usr/share/$l_gdmprofile/greeter-dconf-defaults" > /etc/dconf/profile/$l_gdmprofile
        fi
        
        if [ ! -d "/etc/dconf/db/$l_gdmprofile.d/" ]; then
            echo -e "Creating dconf database directory \"/etc/dconf/db/$l_gdmprofile.d/\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if ! grep -Piq '^\h*idle-delay\h*=\h*uint32\h+([1-9]|[1-9][0-9]|[1-8][0-9]{2}|900)\b' /etc/dconf/db/$l_gdmprofile.d/*; then
            if ! grep -Piq '^\h*idle-delay\h*=\h*' /etc/dconf/db/$l_gdmprofile.d/*; then
                echo -e "creating gdm keyfile for machine-wide settings" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                if ! grep -Piq -- '^\h*\[org\/gnome\/desktop\/session\]' /etc/dconf/db/$l_gdmprofile.d/*; then
                    echo -e "Adding idle-delay entry to \"/etc/dconf/db/$l_gdmprofile.d/00-screensaver\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                    echo -e "\n[org/gnome/desktop/session]\n# Number of seconds of inactivity before the screen goes blank\n# Set to 0 seconds if you want to deactivate the screensaver.\nidle-delay=uint32 900" >> /etc/dconf/db/$l_gdmprofile.d/00-screensaver
                else
                    echo -e "Adding idle-delay value to existing [org/gnome/desktop/session] sections" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                ...
                echo -e "Updating idle-delay value in existing [org/gnome/desktop/session] sections" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if ! grep -Piq '^\h*lock-delay\h*=\h*uint32\h+[0-5]\b' /etc/dconf/db/$l_gdmprofile.d/*; then
            if ! grep -Piq '^\h*lock-delay\h*=\h*' /etc/dconf/db/$l_gdmprofile.d/*; then
                ...
                if ! grep -Piq -- '^\h*\[org\/gnome\/desktop\/screensaver\]' /etc/dconf/db/$l_gdmprofile.d/*; then
                    echo -e "Adding lock-delay entry to \"/etc/dconf/db/$l_gdmprofile.d/00-screensaver\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                    echo -e "\n[org/gnome/desktop/screensaver]\n# Number of seconds after the screen is blank before locking the screen\nlock-delay=uint32 5" >> /etc/dconf/db/$l_gdmprofile.d/00-screensaver
                else
                    echo -e "Adding lock-delay value to existing [org/gnome/desktop/screensaver] sections" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                ...
                echo -e "Updating lock-delay value in existing [org/gnome/desktop/screensaver] sections" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        echo -e "- End remediation - Ensure GDM screen locks when the user is idle" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
    }
