...
#                                                                            #
# File Name:   RHEL9_3.3.1_ensure_source_routed_packets_not_accepted.sh      #
# Description: Ensures source routed packets are not accepted                #
#                                                                            #
##############################################################################
...
ensure_source_routed_packets_not_accepted()
{
	...
	{
		echo "- Start check - Ensure source routed packets are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output="" l_output2="" 
		...
		if [ -z "$l_output2" ]; then 
			...
			echo "- End check - Ensure source routed packets are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo "- Start remediation - Ensure source routed packets are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output="" l_output2="" 
		...
		echo "- End remediation - Ensure source routed packets are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
