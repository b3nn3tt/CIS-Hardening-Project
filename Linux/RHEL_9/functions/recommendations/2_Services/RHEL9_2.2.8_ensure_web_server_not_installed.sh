...
#                                                                        #
# File Name:   RHEL9_2.2.8_ensure_web_server_not_installed.sh            #
# Description: Ensures a web server is not installed                     #
#                                                                        #
##########################################################################
...
ensure_web_server_not_installed()
{
	...
	ensure_web_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ httpd | grep "not installed" && $G_PQ nginx | grep "not installed"; then
						echo -e "- PASSED:\n- Web server packages not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- Web server packages installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ apache2; then	
						...
						echo -e "- FAILED:\n- apache2 package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- apache2 package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure a web server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
