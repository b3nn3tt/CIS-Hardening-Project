...
#                                                                       #
# File Name:   RHEL9_1.6.1.7_fed_ensure_setroubleshoot_not_installed.sh #
...
fed_ensure_setroubleshoot_not_installed()
{
	...
	fed_ensure_setroubleshoot_not_installed_chk()
	{
		...
		if [ -z "$l_output" ]; then
			if $G_PQ setroubleshoot | grep "not installed" > /dev/null; then
				echo -e "- PASSED:\n- setroubleshoot package not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - setroubleshoot" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_PASS:-101}"
			else
				echo -e "- FAILED:\n- setroubleshoot package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - setroubleshoot package" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start Remediation - removing setroubleshoot package" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		$G_PR setroubleshoot
		echo -e "- End Remediation - setroubleshoot" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}	
