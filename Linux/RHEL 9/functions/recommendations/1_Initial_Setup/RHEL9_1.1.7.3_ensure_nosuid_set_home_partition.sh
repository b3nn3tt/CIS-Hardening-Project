...
#                                                                      #
# File Name:   RHEL9_1.1.7.3_ensure_nosuid_set_home_partition.sh       #
# Description: Ensures nosuid option set on /home partition            #
#                                                                      #
########################################################################
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure nosuid option set on /home partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nosuid option set on /home partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure nosuid option set on /home partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure nosuid option set on /home partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
