...
#                                                                            #
# File Name:   RHEL9_2.3.1_ensure_telnet_client_not_installed.sh             #
# Description: Ensures telnet client is not installed                        #
#                                                                            #
##############################################################################
...
ensure_telnet_client_not_installed()
{
	...
	ensure_telnet_client_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ telnet | grep "not installed"; then
						echo -e "- PASSED:\n- telnet package not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- telnet package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ telnet; then	
						...
						echo -e "- PASSED:\n- telnet package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure telnet client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
