...
#                                                                                                    #
# File Name:   RHEL9_1.3.3_fed_ensure_cryptographic_mechanisms_used_protect_integrity_audit_tools.sh #
# Description: Ensures cryptographic mechanisms are used to protect the integrity of audit tools     #
#                                                                                                    #
######################################################################################################
...
fed_ensure_cryptographic_mechanisms_used_protect_integrity_audit_tools()
{
	...
	fed_ensure_cryptographic_mechanisms_used_protect_integrity_audit_tools_chk()
	{
		...
		if [ "$l_test1" = "passed" -a "$l_test2" = "passed" -a "$l_test3" = "passed" -a "$l_test4" = "passed" -a "$l_test5" = "passed" -a "$l_test6" = "passed" ]; then
            echo -e "- PASS:\n- ensure cryptographic mechanisms are used to protect the integrity of audit tools"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		   	echo "- End check - ensure cryptographic mechanisms are used" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		   	return "${XCCDF_RESULT_PASS:-101}"
		else
			echo "- FAILED:"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- ensure cryptographic mechanisms are NOT being used to protect the integrity of audit tools"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		 	echo "- End check - ensure cryptographic mechanisms are used" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo "- Start remediation - ensure cryptographic mechanisms are used to protect the integrity of audit tools" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo "- End remediation - ensure cryptographic mechanisms are used to protect the integrity of audit tools" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"	
	}
