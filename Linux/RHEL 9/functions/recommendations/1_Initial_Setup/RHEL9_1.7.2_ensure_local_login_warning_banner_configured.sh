...
#                                                                          #
# File Name:   RHEL9_1.7.2_ensure_local_login_warning_banner_configured.sh #
# Description: Ensures local login warning banner is configured properly   #
#                                                                          #
############################################################################
...
ensure_local_login_warning_banner_configured()
{
	...
	{
		echo -e "- Start check - Ensure local login warning banner is configured properly" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"

		if [ -e /etc/issue ]; then
			if grep -E -i "(\\\v|\\\r|\\\m|\\\s|$(grep '^ID=' /etc/os-release | cut -d= -f2 | sed -e 's/"//g'))" /etc/issue > /dev/null; then
				# print the reason why we are failing
				echo -e "- FAILED:\n- /etc/issue contains $(grep -E -i "(\\\v|\\\r|\\\m|\\\s|$(grep '^ID=' /etc/os-release | cut -d= -f2 | sed -e 's/"//g'))" /etc/issue)"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure local login warning banner is configured properly" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_FAIL:-102}"
			else
				echo -e "- PASS:\n- /etc/issue is configured properly"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure local login warning banner is configured properly" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
