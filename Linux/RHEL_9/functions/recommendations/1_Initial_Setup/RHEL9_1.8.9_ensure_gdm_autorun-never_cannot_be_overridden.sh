...
#                                                                           #
# File Name:   RHEL9_1.8.9_ensure_gdm_autorun-never_cannot_be_overridden.sh #
# Description: Ensures GDM autorun-never is not overridden                  #
#                                                                           #
#############################################################################
...
ensure_gdm_autorun-never_cannot_be_overridden()
{
	...
	{
		echo "- Start check - Ensure GDM autorun-never is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if [ -n "$l_pkgoutput" ]; then
            ...
            echo -e "- End check - Ensure GDM autorun-never is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_test="NA"
        fi
    ...
    ensure_gdm_autorun-never_cannot_be_overridden_fix()
	{
        ...
        echo -e "- Start remediation - Ensure GDM autorun-never is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        
        # Look for autorun to determine profile in use, needed for remaining tests
        l_kfd="/etc/dconf/db/$(grep -Psril '^\h*autorun-never\b' /etc/dconf/db/*/ | awk -F'/' '{split($(NF-1),a,".");print a[1]}').d" #set directory of key file to be locked
        if [ -d "$l_kfd" ]; then # If key file directory doesn't exist, options can't be locked
            if grep -Priq '^\h*\/org/gnome\/desktop\/media-handling\/autorun-never\b' "$l_kfd"; then
                echo -e " - \"autorun-never\" is locked in \"$(grep -Pril '^\h*\/org/gnome\/desktop\/media-handling\/autorun-never\b' "$l_kfd")\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            else
                echo -e " - creating entry to lock \"autorun-never\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            ...
            echo -e " - \"autorun-never\" is not set so it can not be locked\n - Please follow Recommendation \"Ensure GDM autorun-never is enabled\" and follow this Recommendation again" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        fi
   
        echo -e "- Running 'dconf update'" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        dconf update
      
        echo -e "- End remediation - Ensure GDM autorun-never is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
