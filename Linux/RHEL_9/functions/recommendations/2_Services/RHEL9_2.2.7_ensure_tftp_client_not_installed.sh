...
#                                                                        #
# File Name:   RHEL9_2.2.7_ensure_tftp_client_not_installed.sh           #
# Description: Ensures TFTP Server is not installed                      #
#                                                                        #
##########################################################################
...
ensure_tftp_client_not_installed()
{
	...
	ensure_tftp_client_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure TFTP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ tftp | grep "not installed" ; then
						echo -e "- PASSED:\n- tftp package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure TFTP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- tftp package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure TFTP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure TFTP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure TFTP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
