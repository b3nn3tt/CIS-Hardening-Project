...
#                                                                                #
# File Name:   RHEL9_3.4.1.2_fed_ensure_single_firewall_configuration_utility.sh #
# Description: Ensures a single firewall configuration utility is in use         #
#                                                                                #
##################################################################################
...
fed_ensure_single_firewall_configuration_utility()
{
	...
	fed_ensure_single_firewall_configuration_utility_chk()
	{
        ...
        if [ -z "$l_output2" ]; then
            ...
            echo -e "- PASSED:\n- Ensure single firewall configuration utility in use" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo -e "- End check - single firewall configuration utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
        else
            echo -e "\n- Audit Results:\n ** Fail **\n$l_output2\n" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            echo -e "- FAILED:\n- Ensure single firewall configuration utility in use" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		l_output="" l_output2="" l_fwd_status="" l_nft_status="" l_fwutil_status="" l_pkgmgr=""
        echo "- Start remediation - Ensure single firewall configuration utility in use" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		case $l_fwutil_status in
            enabled:active:masked:inactive|enabled:active:disabled:inactive) 
                echo -e "\n - FirewallD utility is in use, enabled and active\n - NFTables utility is correctly disabled or masked and inactive\n - no remediation required" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG" ;;
            masked:inactive:enabled:active|disabled:inactive:enabled:active) 
                echo -e "\n - NFTables utility is in use, enabled and active\n - FirewallD utility is correctly disabled or masked and inactive\n - no remediation required" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG" ;;
            enabled:active:enabled:active)
                echo -e "\n - Both FirewallD and NFTables utilities are enabled and active\n - stopping and masking NFTables utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                systemctl stop nftables && systemctl --now mask nftables ;;
            disabled:active:disabled:inactive)
                echo -e "\n - Both FirewallD and NFTables utilities are disabled, but FirewallD is active - enabling the FirewallD utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                systemctl enable firewalld ;;
            disabled:active:masked:inactive)
                echo -e "\n - Both FirewallD and NFTables utilities are disabled, but FirewallD is active - enabling the FirewallD utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                systemctl enable firewalld ;;
            disabled|masked:inactive:disabled:active)
                echo -e "\n - Both FirewallD and NFTables utilities are disabled, but NFTables is active - enabling the NFTables utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                systemctl enable nftables ;; 
            disabled:inactive:disabled:inactive)
                echo -e "\n - Both FirewallD and NFTables utilities are disabled and inactive, enabling and starting the FirewallD utility" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                systemctl enable firewalld 
            ...
            enabled:inactive:disabled:inactive)
                ...
                systemctl start firewalld ;;
            enabled:*:enabled:*)
                echo -e "\n - Both FirewallD and NFTables utilities are enabled\n - remediating" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            ...
            *:active:*:active) 
            ...
            :enabled:active) 
                echo -e "\n - NFTables utility is in use, enabled, and active\n - FirewallD package is not installed\n - no remediation required" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG" ;;
            :) 
                echo -e "\n - Neither FirewallD or NFTables is installed.\n - remediating\n - installing NFTables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                $G_PM -y install nftables ;;
            *:*:) 
                echo -e "\n - NFTables package is not installed on the system\n - remediating\n - installing NFTables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
               $G_PM -y install nftables ;;
            *) 
                echo -e "\n - Unable to determine firewall state" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                echo -e "- Verify and configure the appropriate firewall utility based on organizational policies." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			    l_test="manual" ;;
        esac

        echo "- End remediation - Ensure single firewall configuration utility in use" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}	
