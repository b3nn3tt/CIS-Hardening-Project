...
#                                                                            #
# File Name:   RHEL9_2.3.2_ensure_ldap_client_not_installed.sh               #
# Description: Ensures LDAP client is not installed                          #
#                                                                            #
##############################################################################
...
ensure_ldap_client_not_installed()
{
	...
	ensure_ldap_client_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ openldap-clients | grep "not installed"; then
						echo -e "- PASSED:\n- openldap-clients package not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- openldap-clients package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ ldap-utils; then	
						...
						echo -e "- FAILED:\n- ldap-utils package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- ldap-utils package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure LDAP client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
