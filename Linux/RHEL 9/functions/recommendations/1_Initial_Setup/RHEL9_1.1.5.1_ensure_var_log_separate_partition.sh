...
#                                                                      #
# File Name:   RHEL9_1.1.5.1_ensure_var_log_separate_partition.sh      #
# Description: Ensures separate partition exists for /var/log          #
#                                                                      #
########################################################################
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure separate partition exists for /var/log" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_partition_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure separate partition exists for /var/log" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure separate partition exists for /var/log" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure separate partition exists for /var/log" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
