...
#                                                                        #
# File Name:   RHEL9_2.2.5_ensure_dns_server_not_installed.sh            #
# Description: Ensures DNS Server is not installed                       #
#                                                                        #
##########################################################################
...
ensure_dns_server_not_installed()
{
	...
	ensure_dns_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure DNS Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ bind | grep "not installed" ; then
						echo -e "- PASSED:\n- dns server package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure DNS Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- dns server package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure DNS Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure DNS Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure DNS Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
