...
#                                                                      #
# File Name:   RHEL9_1.1.3.3_ensure_nosuid_set_var_partition.sh        #
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure nosuid option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nosuid option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure nosuid option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure nosuid option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
