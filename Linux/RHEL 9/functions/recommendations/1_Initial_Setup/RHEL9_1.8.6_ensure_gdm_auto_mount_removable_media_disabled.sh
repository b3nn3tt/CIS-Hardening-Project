...
#                                                                            #
# File Name:   RHEL9_1.8.6_ensure_gdm_auto_mount_removable_media_disabled.sh #
# Description: Ensures GDM automatic mounting of removable media is disabled #
#                                                                            #
##############################################################################
...
ensure_gdm_auto_mount_removable_media_disabled()
{
	...
	{
		echo "- Start check - Ensure GDM automatic mounting of removable media is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if [ -n "$l_pkgoutput" ]; then
            ...
            echo -e "- End check - Ensure GDM automatic mounting of removable media is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            l_test="NA"
        fi
	...
	{
        echo -e "- Start remediation - Ensure GDM automatic mounting of removable media is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if grep -Pqs -- '^\h*automount-open\h*=\h*false\b' "$l_kfile"; then
            echo -e " - \"automount-open\" is set to false in: \"$l_kfile\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        else
            echo -e " - creating \"automount-open\" entry in \"$l_kfile\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        if grep -Pqs -- '^\h*automount\h*=\h*false\b' "$l_kfile"; then
            echo -e " - \"automount\" is set to false in: \"$l_kfile\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        else
            echo -e " - creating \"automount\" entry in \"$l_kfile\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            ! grep -Psq -- '\^\h*\[org\/gnome\/desktop\/media-handling\]\b' "$l_kfile" && echo '[org/gnome/desktop/media-handling]' >> "$l_kfile"
        ...
        echo -e "- End remediation - Ensure GDM automatic mounting of removable media is disabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
    }
