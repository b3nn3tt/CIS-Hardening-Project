...
#                                                                      #
# File Name:   RHEL9_1.8.1_ensure_gdm_removed.sh                       #
# Description: Ensures GNOME Display Manager is removed                #
#                                                                      #
########################################################################
...
ensure_gdm_removed()
{
    ...
    ensure_gdm_removed_chk()
	{
        ...
        echo -e "- Start check - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ gdm | grep "not installed"; then
						echo -e "- PASSED:\n- Gnome Desktop Manager not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- Gnome Desktop Manager installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ gdm3; then	
						...
						echo -e "- FAILED:\n- Gnome Desktop Manager package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- Gnome Desktop Manager package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
        echo -e "- Start remediation - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        echo -e "- End remediation - Ensure GNOME Display Manager is removed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
