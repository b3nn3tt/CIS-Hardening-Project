...
#                                                                            #
# File Name:   RHEL9_3.3.9_ensure_ipv6_router_advertisements_not_accepted.sh #
# Description: Ensures IPv6 router advertisements are not accepted           #
#                                                                            #
##############################################################################
...
ensure_ipv6_router_advertisements_not_accepted()
{
	...
	{
		echo "- Start check - Ensure IPv6 router advertisements are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output="" l_output2="" 
		...
		if [ -z "$l_output2" ]; then 
			...
			echo "- End check - Ensure IPv6 router advertisements are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo "- Start remediation - Ensure IPv6 router advertisements are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output="" l_output2="" 
		...
		echo "- End remediation - Ensure IPv6 router advertisements are not accepted" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
