...
#                                                                          #
# File Name:   RHEL9_1.8.5_ensure_gdm_screen_locks_cannot_be_overridden.sh #
# Description: Ensures GDM screen locks cannot be overridden               #
#                                                                          #
############################################################################
...
ensure_gdm_screen_locks_cannot_be_overridden()
{
	...
	{
		echo "- Start check - Ensure GDM screen locks cannot be overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        l_pkgoutput=""
	...
	{
        echo -e "- Start remediation - Ensure GDM screen locks cannot be overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"      
        ...
        if [ -d "$l_kfd" ]; then # If key file directory doesn't exist, options can't be locked
            if grep -Prilq '^\h*\/org\/gnome\/desktop\/session\/idle-delay\b' "$l_kfd"; then
                echo -e " - \"idle-delay\" is locked in \"$(grep -Pril '^\h*\/org\/gnome\/desktop\/session\/idle-delay\b' "$l_kfd")\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            else
                echo -e "creating entry to lock \"idle-delay\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                [ ! -d "$l_kfd"/locks ] && echo "creating directory $l_kfd/locks" && mkdir "$l_kfd"/locks
                {
                echo -e '\n# Lock desktop screensaver idle-delay setting' | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                echo '/org/gnome/desktop/session/idle-delay'
                } >> "$l_kfd"/locks/00-screensaver        
            fi
        else
            echo -e " - \"idle-delay\" is not set so it can not be locked\n - Please follow Recommendation \"Ensure GDM screen locks when the user is idle\" and follow this Recommendation again" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        fi
        
        if [ -d "$l_kfd2" ]; then # If key file directory doesn't exist, options can't be locked
            if grep -Prilq '^\h*\/org\/gnome\/desktop\/screensaver\/lock-delay\b' "$l_kfd2"; then
                echo -e " - \"lock-delay\" is locked in \"$(grep -Pril '^\h*\/org\/gnome\/desktop\/screensaver\/lock-delay\b' "$l_kfd2")\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            else
                echo -e "creating entry to lock \"lock-delay\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                [ ! -d "$l_kfd2"/locks ] && echo "creating directory $l_kfd2/locks" && mkdir "$l_kfd2"/locks
                {
                echo -e '\n# Lock desktop screensaver lock-delay setting' | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        echo -e "- End remediation - Ensure GDM screen locks cannot be overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
    }
