...
#                                                                        #
# File Name:   RHEL9_2.2.14_fed_ensure_dnsmasq_not_installed.sh          #
# Description: Ensures dnsmasq is not installed                          #
#                                                                        #
##########################################################################
...
fed_ensure_dnsmasq_not_installed()
{
	...
	fed_ensure_dnsmasq_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure dnsmasq is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ dnsmasq | grep "not installed" ; then
						echo -e "- PASSED:\n- dnsmasq package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure dnsmasq is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- dnsmasq package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure dnsmasq is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure dnsmasq is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure dnsmasq is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
