...
#                                                                        #
# File Name:   RHEL9_2.2.13_fed_ensure_telnet_server_not_installed.sh    #
# Description: Ensures telnet-server is not installed                    #
#                                                                        #
##########################################################################
...
fed_ensure_telnet_server_not_installed()
{
	...
	{
		echo -e "- Start check - Ensure telnet-server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output=""
		l_pkgmgr=""
		...
		if [ -z "$l_output" ]; then
			if $G_PQ telnet-server | grep "not installed"; then	
				echo -e "- PASSED:\n- telnet server not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure telnet-server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_PASS:-101}"
			else
				echo -e "- FAILED:\n- telnet server installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure telnet-server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure telnet-server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		
		$G_PR telnet-server

		echo -e "- End remediation - Ensure telnet-server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
