...
#                                                                        #
# File Name:   RHEL9_2.2.3_ensure_cups_not_installed.sh                  #
# Description: Ensures CUPS is not enabled                               #
#                                                                        #
##########################################################################
...
ensure_cups_not_installed()
{
	...
	ensure_cups_not_installed_chk()
	{
        ...
        echo -e "- Start check - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ cups | grep "not installed" && $G_PQ avahi | grep "not installed"; then
						echo -e "- PASSED:\n- CUPS not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- CUPS installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ cups; then	
						...
						echo -e "- FAILED:\n- CUPS package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- CUPS package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
        echo -e "- Start remediation - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        ...
        echo -e "- End remediation - Ensure CUPS is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
