...
#                                                                        #
# File Name:   RHEL9_1.1.6.3_ensure_nodev_set_var_log_audit_partition.sh #
# Description: Ensures nodev option set on /var/log/audit partition      #
#                                                                        #
##########################################################################
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure nodev option set on /var/log/audit partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nodev option set on /var/log/audit partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure nodev option set on /var/log/audit partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure nodev option set on /var/log/audit partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
