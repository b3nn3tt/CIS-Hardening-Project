...
#                                                                            #
# File Name:   RHEL9_3.4.2.1_fed_ensure_firewalld_default_zone_set.sh        #
# Description: Ensures firewalld default zone is set                         #
#                                                                            #
##############################################################################
...
fed_ensure_firewalld_default_zone_set()
{
	...
	{
		echo "- Start check - Is the firewalld default zone set" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output=""
		...
		if [ -z "$l_output" ]; then
			echo -e "- The firewalld default zone is: \"$l_fwddz\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Is the firewalld default zone set" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
	...
	{
		echo -e "- Start remediation - Set firewalld default zone" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		echo -e "- Setting default zone to \"public\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		firewall-cmd --set-default-zone=public
		echo -e "- End remediation - Set firewalld default zone"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
