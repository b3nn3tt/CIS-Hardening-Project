...
#                                                                      #
# File Name:   RHEL9_1.8.10_fed_ensure_xdmcp_not_enabled.sh            #
# Description: Ensures XDMCP is not enabled                            #
#                                                                      #
########################################################################
...
fed_ensure_xdmcp_not_enabled()
{
	...
	gnome_installed_chk()
	{
		echo "- Start check - Is GNOME Display Manager Installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		l_output=""
		l_pkgmgr=""
		...
		if [ -z "$l_pkgmgr" ]; then
			...
			echo -e "- Checking for GNOME Display Manager package" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		if [ -n "$l_pkgmgr" ] ; then
			echo -e "- FAILED: $l_pkgmgr" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            echo -e "- End check - Is GNOME Display Manager Installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            return "${XCCDF_RESULT_PASS:-106}"
		elif [ -z "$l_output" ] ; then
			echo -e "- The GNOME Display Manager package is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo -e "- End check - Is GNOME Display Manager Installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo -e "- The GNOME Display Manager package is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo -e "- End check - Is GNOME Display Manager Installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	...
	{
		if [ "$l_test" != "NA" ] ; then
			echo -e "- Start check - Ensure XDMCP is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			if ! grep -Pq -- '^\h*Enable\h*=\h*true' /etc/gdm/custom.conf; then
				echo -e "- PASS: - 'Enable=true' was NOT found in /etc/gdm/custom.conf." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure XDMCP is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_PASS:-101}"
			else
				echo -e "- FAIL: - 'Enable=true' was found in /etc/gdm/custom.conf."  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure XDMCP is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure XDMCP is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"

		echo -e "- Removing 'Enabled=true from /etc/gdm/custom.conf"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		sed -ri 's/^\s*(Enable\s*=\s*true)/#\1/' /etc/gdm/custom.conf
			
		echo -e "- End remediation - Ensure XDMCP is not enabled" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
