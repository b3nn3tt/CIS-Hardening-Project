...
#                                                                      #
# File Name:   RHEL9_1.1.3.2_ensure_nodev_set_var_partition.sh         #
# Description: Ensures nodev option set on /var partition              #
#                                                                      #
########################################################################
...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start check - Ensure nodev option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ "$l_current_test" = "passed" ] && [ "$l_mount_test" = "passed" ]; then
         ...
         echo -e "- End check - Ensure nodev option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   ...
   {
      echo -e "- Start remediation - Ensure nodev option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            ...
            for l_dname in $l_unit_files; do
               ...
               if [ -n "$l_mfile" ] && systemctl list-unit-files --type mount "$l_mfile" &>/dev/null; then
                  if ! awk '/[Mount]/,0' $l_dname | grep -Eq "^\s*Options=([^#]+,)?$l_option"; then
                     ...
                     echo -e "- Remounting $XCCDF_VALUE_REGEX with nodev,nosuid" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      echo -e "- End remediation - Ensure nodev option set on /var partition" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
   }
