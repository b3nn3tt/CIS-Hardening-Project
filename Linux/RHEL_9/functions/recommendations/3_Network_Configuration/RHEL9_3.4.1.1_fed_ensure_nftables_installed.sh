...
#                                                                            #
# File Name:   RHEL9_3.4.1.1_fed_ensure_nftables_installed.sh                #
# Description: Ensures nftables is installed                                 #
#                                                                            #
##############################################################################
...
fed_ensure_nftables_installed()
{
	...
	{
		echo "- Start check - Ensure nftables is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		output=""
		...
		if [ -z "$output" ] ; then
			echo "- The nftables package is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure nftables is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo -e "$output" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure nftables is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	...
	{
		echo -e "- Start remediation - Ensure nftables is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		$G_PM -y install nftables
		echo -e "- End remediation - Ensure nftables is installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
