...
#                                                                        #
# File Name:   RHEL9_2.2.2_ensure_avahi_server_not_installed.sh          #
# Description: Ensures Avahi Server is not installed                     #
#                                                                        #
##########################################################################
...
ensure_avahi_server_not_installed()
{
	...
	ensure_avahi_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ avahi-autopid | grep "not installed" && $G_PQ avahi | grep "not installed"; then
						echo -e "- PASSED:\n- avahi packages not found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- avahi packages installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				...
				*dpkg*)
					if $G_PQ avahi-daemon; then	
						...
						echo -e "- FAILED:\n- avahi server package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-102}"
					else
						echo -e "- PASSED:\n- avahi server package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					fi
				;;
	...
	{
		echo -e "- Start remediation - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"

		case "$G_PQ" in 
			*rpm*)
				echo -e "- Stopping service" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure Avahi Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
