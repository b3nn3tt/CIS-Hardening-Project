...
#                                                                            #
# File Name:   RHEL9_3.4.2.3_fed_ensure_nftables_base_chains_exist.sh        #
# Description: Ensures nftables base chains exist                            #
#                                                                            #
##############################################################################
...
fed_ensure_nftables_base_chains_exist()
{
	...
	{
		echo "- Start check - Ensure an nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      ...
      if [ -n "$l_input_chain" ]; then
         echo "- Found INPUT chain" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         if [ -n "$l_forward_chain" ]; then
            echo "- Found FORWARD chain" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            if [ -n "$l_output_chain" ]; then
               echo "- Found OUTPUT chain" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
               l_ruleset=passed
            else
               echo "- NO OUTPUT chain found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            fi
         else
            echo "- NO FORWARD chain found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
         fi
      else
         echo "- NO INPUT chain found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      fi
      
      # if ruleset passes, we pass
		if [ "$l_ruleset" = passed ] ; then
			echo "- An INPUT, FORWARD and OUTPUT chain were found" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure firewalld is either not installed or masked with iptables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_PASS:-101}"
		else
			# print the reason why we are failing
			echo "- Some chains were missing" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "- End check - Ensure firewalld is either not installed or masked with iptables" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			return "${XCCDF_RESULT_FAIL:-102}"
		fi
	}
	...
	{
		echo -e "- Start remediation - Ensure an nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		
      echo -e "- Run \"nft create chain inet <table name> <base chain name> { type filter hook <(input|forward|output)> priority 0 \; }\" to create a table according to your local site policies." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
      test=manual
      
		echo -e "- End remediation - Ensure an nftables table exists" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
