...
#                                                                           #
# File Name:   RHEL9_1.3.2_ensure_filesystem_integrity_regularly_checked.sh #
# Description: Ensures filesystem integrity is regularly checked            #
#                                                                           #
#############################################################################
...
ensure_filesystem_integrity_regularly_checked()
{
	...
	ensure_filesystem_integrity_regularly_checked_chk()
   	{
		echo -e "- Start check - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		if $G_PQ aide > /dev/null 2>&1 && $G_PQ aide-common > /dev/null 2>&1; then
			...
			if [ "$l_cron_test" = "passed" ] || [ "$l_systemctl_test" = "passed" ]; then
				echo -e "- PASS:\n- Filesystem integrity is being checked"  | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_PASS:-101}"
			else
				echo -e "- FAIL:\n- Filesystem integrity is NOT being checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				echo -e "- End check - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
				return "${XCCDF_RESULT_FAIL:-102}"
			fi
		else
			echo -e "- aide packages NOT installed on the system." | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo -e "- End check - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			l_test="manual"
	...
	ensure_filesystem_integrity_regularly_checked_fix()
   	{
      	echo -e "- Start remediation - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"

		if [ "$l_test" != "manual" ]; then
			echo -e "- Adding '/usr/sbin/aide --check' to root's crontab" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			echo "0 5 * * * /usr/sbin/aide --check" | crontab -u root -
		else
			echo -e "- Install the aide package as appropriate for your environment then add the following to the crontab:\n  0 5 * * * /usr/bin/aide.wrapper --config /etc/aide/aide.conf --check" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		fi

		echo -e "- End remediation - Ensure filesystem integrity is regularly checked" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
