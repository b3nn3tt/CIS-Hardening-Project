...
#                                                                            #
# File Name:   RHEL9_2.3.4_ensure_ftp_client_not_installed.sh                #
# Description: Ensures FTP Client is not installed                           #
#                                                                            #
##############################################################################
...
ensure_ftp_client_not_installed()
{
	...
	ensure_ftp_client_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure FTP Client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ ftp | grep "not installed" ; then
						echo -e "- PASSED:\n- ftp client package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure FTP Client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- ftp client package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure FTP Client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure FTP Client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure FTP Client is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
