...
#                                                                        #
# File Name:   RHEL9_2.2.6_ensure_vsftp_server_not_installed.sh          #
# Description: Ensures VSFTP Server is not installed                     #
#                                                                        #
##########################################################################
...
ensure_vsftp_server_not_installed()
{
	...
	ensure_vsftp_server_not_installed_chk()
	{
		...
		echo -e "- Start check - Ensure VSFTP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
			...
			case "$G_PQ" in 
				*rpm*)
					if $G_PQ vsftpd | grep "not installed" ; then
						echo -e "- PASSED:\n- vsftpd package not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure VSFTP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						return "${XCCDF_RESULT_PASS:-101}"
					else
						echo -e "- FAILED:\n- vsftpd package installed on the system" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
						echo -e "- End check - Ensure VSFTP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
		echo -e "- Start remediation - Ensure VSFTP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
		...
		echo -e "- End remediation - Ensure VSFTP Server is not installed" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	}
