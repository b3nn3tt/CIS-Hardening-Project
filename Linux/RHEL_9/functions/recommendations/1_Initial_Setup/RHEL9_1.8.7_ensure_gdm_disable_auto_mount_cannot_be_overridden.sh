...
#                                                                                            #
# File Name:   RHEL9_1.8.7_ensure_gdm_disable_auto_mount_cannot_be_overridden.sh             #
...
ensure_gdm_disable_auto_mount_cannot_be_overridden()
{
	...
	{
		echo "- Start check - Ensure GDM disabling automatic mounting of removable media is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        l_pkgoutput=""
        ...
        if [ -z "$l_output2" ]; then
            echo -e "\n- Audit Result:\n  ** PASS **\n$l_output\n"
            echo -e "- End check - Ensure GDM screen locks cannot be overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
	...
	{
        echo -e "- Start remediation - Ensure GDM disabling automatic mounting of removable media is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"      
        ...
        if [ -d "$l_kfd" ]; then # If key file directory doesn't exist, options can't be locked
            if grep -Priq '^\h*\/org/gnome\/desktop\/media-handling\/automount\b' "$l_kfd"; then
                echo -e " - \"automount\" is locked in \"$(grep -Pril '^\h*\/org/gnome\/desktop\/media-handling\/automount\b' "$l_kfd")\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            else
                echo -e " - creating entry to lock \"automount\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                [ ! -d "$l_kfd"/locks ] && echo -e "- Creating directory $l_kfd/locks" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG" && mkdir "$l_kfd"/locks
                {
            ...
            echo -e " - \"automount\" is not set so it can not be locked\n - Please follow Recommendation \"Ensure GDM automatic mounting of removable media is disabled\" and follow this Recommendation again" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
        fi
        
        if [ -d "$l_kfd2" ]; then # If key file directory doesn't exist, options can't be locked
            if grep -Priq '^\h*\/org/gnome\/desktop\/media-handling\/automount-open\b' "$l_kfd2"; then
                echo -e " - \"automount-open\" is locked in \"$(grep -Pril '^\h*\/org/gnome\/desktop\/media-handling\/automount-open\b' "$l_kfd2")\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
            else
                echo -e " - creating entry to lock \"automount-open\"" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
                [ ! -d "$l_kfd2"/locks ] && echo -e "- Creating directory $l_kfd2/locks" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG" && mkdir "$l_kfd2"/locks
                {
        ...
        echo -e "- End remediation - Ensure GDM disabling automatic mounting of removable media is not overridden" | tee -a "$VERBOSE_LOG" 2>> "$ERROR_LOG"
    }
